...
2. **Header branding is static; never overwritten by the facility name.** The header block (web UI header and top page of every export) is: the static **INFINITE — Managed by MEDELITE logo image** (`src/lib/report/logo.ts`, sourced from the official template's `image1.png`; rendered in both the web preview and the PDF — its alt/label text is the static string `"INFINITE — Managed by MEDELITE"`, which is also `vm.header.platformLine`), then the static header `"FACILITY ASSESSMENT SNAPSHOT"`, then the **dynamic state abbreviation** (e.g. `FL`). The platform brand `"INFINITE"` is never derived from or replaced by the facility name (CMS legal name or manual override). `assembleHeader()` takes **no facility-name argument** (a state code is fine). The facility name — including the optional manual override — appears **only in the report body** under "Name of Facility".
